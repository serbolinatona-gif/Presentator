...
СТРУКТУРНЫЕ кавычки между элементами массива (например ["a\",\"b"] вместо ["a","b"]).
Поэтому промпт прямо запрещает использовать кавычки внутри текста вообще, а репар-парсер
отдельно чинит именно этот паттерн. Также исправлен баг извлечения JSON: раньше код всегда
искал "[" вместо "{", если в тексте вообще был массив — а почти все схемы раскладок содержат
вложенный массив ("items": [...], "facts": [...] и т.д.), из-за чего обрезалось начало объекта.
"""
