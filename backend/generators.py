    ...
    while remaining > 0:
        ...
        def build_prompt(prev_error, batch_size=batch_size, start_index=start_index):
            ...
            return (
                ...
                "НИКОГДА не придумывай сравнения с реальными знаменитостями/людьми — только о самом предмете. "
...
def _common_header(topic: str, style: str, language: str, detailed_prompt: Optional[str]) -> str:
    ...
    return (
        ...
        "конкретики, чем недостоверная. НИКОГДА не придумывай сравнения персонажей/тем с реальными "
        "знаменитостями, актёрами или людьми (например 'похож на X', 'ассоциируется с Y') — это "
        "почти всегда выдумка и вводит в заблуждение. Пиши только о самом предмете презентации. "
